...
!!! important
    ...
    All tag fields *should* be unique in order for the generated script to work correctly, even though PluralityUtilities wont take issue with it. If a tag field is repeated, only the first one in the script will work.
...
???+ example
    ...
    ```
    C:\path\to\tool\folder\PluralityUtilities> PluralityUtilities.exe
    ```
...
!!! important
    ...
    If you pass the path to an existing output file, it will be overwritten.

???+ example

    ```
    C:\path\to\tool\folder\PluralityUtilities> PluralityUtilities.exe C:\path\to\input\file\input.txt C:\path\to\templates\file\templates.txt C:\path\to\output\file\output
    ```
...
???+ example
    ...
    in command prompt:
    ```
    C:\path\to\parent\PluralityUtilities> PluralityUtilities.exe ../input.txt ../templates.txt ./output
    ```
