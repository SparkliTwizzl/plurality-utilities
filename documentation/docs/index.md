...
---
<h1 align="center">Petrichor</h1>
<h2 align="center">Version 0.10.0</h2>
...
???+ example
    ...
    ```powershell
    [install path]\Petrichor> Petrichor.exe --inputFile input.txt
    ```
...
???+ example
    ...
    ```powershell
    [install path]\Petrichor> Petrichor.exe
    ```
...
???+ example
    ...
    ```powershell
    [install path]\Petrichor> Petrichor.exe [path]\input.txt
    ```
...
???+ example
    ...
    === "Full file path"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --inputFile "[path]\inputFile.txt"
        ```
    ...
    === "Directory only, default file name"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --inputFile "[path]\"
        ```
    ...
    === "File name only, default directory"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --inputFile "inputFile.txt"
        ```
...
A file extension is not required, and if one is included it will be replaced automatically. [Relative file paths](#433---relative-file-paths) can be used.
...
???+ example
    ...
    === "Full file path"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --outputFile "C:\path\to\output\file\outputFile"
        ```
    ...
    === "Directory only, default file name"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --outputFile "outputFile"
        ```
    ...
    === "File name only, default directory"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --outputFile "{path}\output\"
        ```
...
!!! note
    ...
    Log file will only be created if logging to file is enabled (see 4.3.3.3).
...
???+ example
    ...
    === "Full file path"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --logFile logFile.txt
        ```
    ...
    === "Directory only, default file name"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --logMode all --logFile [log path]/
        ```
    ...
    === "File name only, default directory"
        ...
        ```powershell
        [install path]\Petrichor> Petrichor.exe commandName --logMode all --logFile [log path]/logFile.txt
        ```
...
!!! note
    ...
    [Relative paths](#433---relative-file-paths) are relative to the batch script by default. If relative paths are used in Petrichor commands, they must be relative to Petrichor.exe instead.
...
???+ example
    ...
    === "Command passed with command line arguments"
...
!!! note
    ...
    You cannot use `::` in a `find` string due to the way AutoHotkey hotstrings work. Petrichor will allow it, but the macros it generates will not work.
...
!!! note
    ...
    By default, you cannot use the `[` or `]` symbols in a template string. Use [escape characters](#413---escape-characters) to circumvent this.
