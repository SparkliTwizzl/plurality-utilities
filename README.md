...
##### 4.2.2.1 - generateAHKShortcutScript command `--inputFile` option (REQUIRED)
...
```
{path to tool folder}\Petrichor\
>Petrichor.exe generateAHKShortcutScript --inputFile C:\path\to\input\file\inputFile.txt
```

##### 4.2.2.2 - generateAHKShortcutScript command `--outputFile` option
...
Petrichor\Petrichor.exe generateAHKShortcutScript --inputFile ..\inputFile.txt --outputFile ..\outputFile
...
### 5.1 - Before you can do anything with your script, you need to install AutoHotkey

Download AutoHotkey v2 [here](https://www.autohotkey.com) and install it, then continue.
...
**DO NOT DO THIS UNLESS YOU KNOW WHAT YOU'RE DOING.** Editing the registry can brick your computer if you're not careful. Wi recommend using one of the other methods above, unless all of them dont work for you.
