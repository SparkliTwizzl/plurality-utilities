...
Add the `--inputFile` option to the `generateTextShortcutScript` command and pass the path to the input file after it. You must include the path and the file extension for the input file. [Relative file paths](#4.2.2---relative-file-paths) can be used.
...
Add the `--outout` option to the `generateTextShortcutScript` command and pass the output file after it.
